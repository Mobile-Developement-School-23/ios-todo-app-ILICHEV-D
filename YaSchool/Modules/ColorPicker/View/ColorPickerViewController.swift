...
class ColorPickerView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        for y in stride(from: 0, to: Int(bounds.height), by: 1) {
            for x in stride(from: 0, to: Int(bounds.width), by: 1) {
                
                let normalizedX = CGFloat(x) / bounds.width
                let normalizedY = CGFloat(y) / bounds.height
                
                let hue = CGFloat(normalizedX)
                let saturation = 1.0 - CGFloat(normalizedY)
                let brightness = 1.0 - CGFloat(y) / bounds.height
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if touchPoint.x <= frame.width, 0 <= touchPoint.x, touchPoint.y <= frame.height, 0 <= touchPoint.y {
            ...
            pointer.frame = .init(x: pointerX, y: (touchPoint.y - pointerWidth / 2), width: pointerWidth, height: pointerWidth)
